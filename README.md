...
I score each run as a weighted line-item vector, not a bare pass/fail — the editor needs a gradient, the
scorecard needs an honest headline. The definition lives in `src/rubric/rubric.ts`.
...
## The mock comes first

The default is a deterministic mock of the Coinflow surface (`src/mock/`): a stateful oracle (issues real
`paymentId`s, checks the reference field, trips a real `410` by controlling the velocity config), a
contract-faithful stub of `@coinflow/react`, and a mock-served card-entry iframe. That keeps the loop
deterministic, offline, CI-safe, and zero-blast-radius — an autonomous loop never hammers the live sandbox.
Live mode swaps the same integration code onto the real SDK and sandbox as a validation adapter: the code
doesn't change, only what sits under it.
...
For live models I supply keys however I like; the loop auto-loads a repo-root `.env`, and exported vars or
a secrets manager override it:
...
The same integration and harness run against any Coinflow environment through env vars — `mock` is the
default and needs no keys. `resolveCoinflowEnv()` (`src/config/coinflow-env.ts`) is the single seam;
`merchantId`, `env`, and `apiKey` flow from it through the contract into the integration.
...
I validate the mock's contract against the real sandbox API (needs the key):

```bash
COINFLOW_MODE=sandbox COINFLOW_API_KEY=… pnpm probe
```

Sandbox/prod **browser** runs additionally need the real `@coinflow/react` installed and the live
hosted-iframe selectors (a documented extension); the mock path and `pnpm probe` need neither.

## The model-agnostic runner

Every provider implements one interface (`src/runner/provider.ts`), and a minimal agentic tool-use loop
runs on top (`src/harness/agent.ts`). Adding a frontier model is a line in `src/runner/registry.ts`, not a
...
`docs/worked-example/` has the before/after, the doc diff, live scorecards with full run dirs (`runs/`), and
the live-sandbox probe. The headline: panel and held-out model both **0.70 → 1.00**, and **live Claude
0.70 → 1.00**, from a three-section, in-budget edit. The honest counterpoint: **live GPT-4o stayed
0.50 → 0.50** — it learned the edits but hallucinates the COF endpoint, a gap the mock panel never showed.
That divergence is the case for real, diverse models in the panel.

Design rationale, brittleness, cost/latency, and CI wiring live in **`docs/writeup.md`**.
